...
pack .qms_show_hide -in .f -fill x
...
pack .qms.lang.values -side right -padx {3 0}
...
pack .qms.fontsize.value -side right
pack .qms.fontfamily -fill x -pady 1
pack .qms.fontfamily.values -side right -padx {3 0}
} else {
set qms.fontsize ""
set qms.fontfamily ""
}
...
proc write_mapsforge {task} {
  ...
  set data {}
  lappend data <TMS>
  lappend data {<Layer idx="0">}
  lappend data <Title>$name</Title>
  set min [expr 21-$::max_zoom_level]
  set max [expr 21-$::min_zoom_level]
  lappend data <MinZoomLevel>$min</MinZoomLevel>
  lappend data <MaxZoomLevel>$max</MaxZoomLevel>
  set url http://127.0.0.1:${::tcp.port}/%1/%2/%3.png?task=$task
  if {$::tile_size != 256} {append url &amp\;tileRenderSize=$::tile_size}
  lappend data <ServerUrl>$url</ServerUrl>
  lappend data </Layer>
  lappend data </TMS>
...
proc srv_task_create {task} {
  foreach item [list ${::task.active} $task] {
    set file $::ini_folder/task.$item.ini
...
proc qms_stop {} {
  ...
  if {$::tcl_platform(os) == "Windows NT"} {
  ...
  } elseif {$::qms_version > 11900} {	# "Linux" / "Darwin"
    ...
    set script "tell application \"QMapShack\" to quit saving yes"
...
if {$tcl_platform(os) == "Windows NT"} {
  ...
  if {[llength $result] == 5} {
	...
	-message [mc e01 QMapShack $exe $pid] -detail [mc e02]] == "no"} exit
...
if {${qms.conf} && ${qms.file} != ""} {
  ...
  switch $tcl_platform(os) {
    "Windows NT" -
    "Darwin"	 {unset -nocomplain file}
    "Linux"	 {set file $env(HOME)/.config/QLandkarte/QMapShack.conf}
  }
}
...
if {[info exists file]} {
  ...
  if {![catch "exec defaults read org.qlandkarte.QMapShack" result]} {
...
tk_busy hold .qms -cursor X_cursor
...
if {[process_running srv]} {qms_start}
busy_state 0
...
qms_stop
if {[process_running qms]} {process_kill qms}
